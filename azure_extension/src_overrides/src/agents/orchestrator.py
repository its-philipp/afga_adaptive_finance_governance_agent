...
try:
    from ..services.databricks_lakehouse import DatabricksLakehouseUploader
except Exception:  # pragma: no cover - optional dependency
    DatabricksLakehouseUploader = None  # type: ignore
...
class AFGAOrchestrator:
    ...
    def __init__(
        ...
        self.databricks_enabled = (
            getattr(self.settings, "memory_backend", "local").lower() == "databricks"
        )
        self.lakehouse_uploader = None
        if self.databricks_enabled and DatabricksLakehouseUploader:
            try:
                if not self.settings.azure_storage_account:
                    raise ValueError("AZURE_STORAGE_ACCOUNT is required for Databricks mode")
                container = getattr(
                    self.settings,
                    "azure_container_raw",
                    "raw-transactions",
                )
                self.lakehouse_uploader = DatabricksLakehouseUploader(
                    storage_account_name=self.settings.azure_storage_account,
                    container_name=container,
                    use_managed_identity=getattr(
                        self.settings, "azure_use_managed_identity", True
                    ),
                    storage_account_key=getattr(self.settings, "azure_storage_key", None),
                    databricks_workspace_url=getattr(
                        self.settings, "databricks_workspace_url", None
                    ),
                    databricks_job_id=getattr(self.settings, "databricks_job_id", None),
                    databricks_token=getattr(self.settings, "databricks_token", None),
                )
                logger.info("Databricks lakehouse uploader initialized")
            except Exception as exc:  # pragma: no cover - best effort
                logger.warning(
                    "Failed to initialize Databricks lakehouse uploader: %s. "
                    "Proceeding with local-only backend.",
                    exc,
                )
                self.lakehouse_uploader = None
                self.databricks_enabled = False
        elif self.databricks_enabled:
            logger.warning(
                "Databricks lakehouse support requested but dependencies are missing. "
                "Install azure-identity, azure-storage-file-datalake, databricks-sdk."
            )
            self.databricks_enabled = False
    ...
    def process_transaction(
        ...
        with self.observability.trace(
            ...
            self._sync_to_lakehouse(result)
    ...
    def process_hitl_feedback(
        ...
        with self.observability.trace(
            ...
            self._sync_hitl_feedback(feedback, invoice)
    ...
    # ------------------------------------------------------------------ #
    # Databricks Lakehouse helpers
    # ------------------------------------------------------------------ #

    def _sync_to_lakehouse(self, result: TransactionResult) -> None:
        """Upload transaction artifacts to Databricks lakehouse if enabled."""
        if not self.lakehouse_uploader:
            return
        try:
            self.lakehouse_uploader.upload_transaction(result)
        except Exception as exc:  # pragma: no cover - non-critical
            logger.warning(
                "Failed to replicate transaction %s to Databricks lakehouse: %s",
                result.transaction_id,
                exc,
            )

    def _sync_hitl_feedback(self, feedback: HITLFeedback, invoice: Invoice) -> None:
        """Replicate human-in-the-loop feedback to lakehouse."""
        if not self.lakehouse_uploader:
            return
        try:
            self.lakehouse_uploader.upload_hitl_feedback(feedback, invoice)
        except Exception as exc:  # pragma: no cover - non-critical
            logger.warning(
                "Failed to replicate HITL feedback %s to Databricks lakehouse: %s",
