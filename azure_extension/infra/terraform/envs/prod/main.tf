terraform {
  ...
  backend "azurerm" {
    ...
    storage_account_name = "philippsstorageaccount"  # Same backend as dev/staging
    container_name       = "tfstate"
    key                  = "adaptive-finance-governance-rag/prod/terraform.tfstate"
  }
...
module "storage" {
  ...
  location                = var.location
  enable_public_access    = false  # Phase 2: Production should use private endpoints
  enable_private_endpoint  = true  # Enable private endpoint for production
